...
def generate_couplings(n, rng: Optional[np.random.RandomState] = None):
    '''
    ...
    J_right = np.array([[rng.choice([+1,-1]) for j in range(n-1)] for i in range(n)])
    J_down = np.array([[rng.choice([+1,-1]) for j in range(n)] for i in range(n-1)])

    return J_right, J_down


def hamiltonian(n, J_right, J_down):
    '''
...
def ground_state_expectation_value(n, hamiltonian, observable):
    '''
    ...
    obs_matrix = qml.matrix(observable, wire_order=[i for i in range(n*n)])
...
def generate_training_set(n, num_examples, observable_name):
    '''
    ...
    J_len = int(2*n*n - 2*n)  # Number of couplings = len(J_right) + len(J_down)
    X = np.zeros((num_examples, J_len))
    Y = np.zeros(num_examples)
    for l in range(num_examples):
        J_right, J_down = generate_couplings(n, rng)
        J_right_flat = J_right.flatten()
        J_down_flat = J_down.flatten()
        X[l,:] = np.concatenate((J_right_flat, J_down_flat))
        Y[l] = ground_state_expectation_value(n, hamiltonian(n, J_right, J_down), obs)
